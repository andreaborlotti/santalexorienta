...
export const ConfrontoOSAStatale: React.FC<ConfrontoOSAStataleProps> = ({ liceo, showApprofondimenti = true, isCompact = false }) => {
  ...
  return (
    ...
    >
      <div className={`border-b ${isCompact ? 'pb-3' : 'pb-4'}`} style={{ borderColor: GIALLO }}>
      ...
      {liceo.cambridgeMaterie && liceo.cambridgeMaterie.length > 0 && (
        <div className={`border-b ${isCompact ? 'pb-3' : 'pb-4'}`} style={{ borderColor: GIALLO }}>
          ...
          <div className="flex flex-wrap gap-2">
            {liceo.cambridgeMaterie.map((m, i) => (<Badge key={i}>{m}</Badge>))}
          </div>
        </div>
      )}
